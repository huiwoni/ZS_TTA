...
class ClipFixed(nn.Module):
    ...
    def reset_classnames(self, classnames, test_sets):
        ...
        if self.text_prompt_type == 'simple':
            ...
            f = open('/home/huiwon/DMN/data/gpt3_prompts/' + self.cupl_file)
...
class Dummy_ClipFixed(nn.Module):
    def __init__(self, device, classnames, batch_size, criterion='cosine', arch="ViT-L/14",
                        n_ctx=16, ctx_init=None, ctx_position='end', learned_cls=False, memory_size=10, text_prompt='tip'):
        super(Dummy_ClipFixed, self).__init__()
        clip, _, transform = load(arch, device=device, download_root=DOWNLOAD_ROOT)
        print('clip transform', transform)
        self.clip = clip
        self.classnames = [name.replace("_", " ") for name in classnames]
    ...
    def reset_classnames(self, classnames, test_sets):
        ...
        if self.text_prompt_type == 'simple':
            ...
            f = open('/home/huiwon/DMN/data/gpt3_prompts/' + self.cupl_file)
            self.cupl_prompts = json.load(f)
        elif self.text_prompt_type == 'desciption_prompt':
            ################################################################ desciption_prompt st
            if len(test_sets)>1:
                self.text_prompt = ID_to_prompts[test_sets.lower()]
                self.cupl_file = ID_to_gptprompts_path[test_sets.lower()]
            else:
                self.text_prompt = tip_imagenet_templates 
            f = open('/home/huiwon/DMN/data/gpt3_prompts/imagenet_class_descriptions.json')
            self.cupl_prompts = json.load(f)
            ################################################################ desciption_prompt ed
        elif self.text_prompt_type == 'full':
            self.text_prompt = imagenet_templates
        else:
            raise NotImplementedError
    ...
    def get_text_features(self):
        ...
        for name in self.classnames:
            ...
            text_feat.append(class_embedding_mean) ### 1024
            full_text_feat.append(class_embeddings_full)
        ...
        self.text_feat_full = torch.stack(full_text_feat, dim=0).cuda()
    ...
    def dummy_get_text_features(self):
        ## get the text feature only once, multiple class & multiple prompt
        text_feat = []
        text_label = []
        full_text_feat = []
        ...
        for name in self.classnames:
            text_prompts = [template.format(name) for template in self.text_prompt]  # format with class
            prompt_num = len(text_prompts)
            if self.text_prompt_type =='tip_cupl' or self.text_prompt_type == 'desciption_prompt':
                text_prompts += self.cupl_prompts[name]
            texts = tokenize(text_prompts).cuda()  # tokenize
            class_embeddings = self.clip.encode_text(texts)  # embed with text encoder
            class_embeddings_full = class_embeddings / class_embeddings.norm(dim=-1, keepdim=True)
            class_embedding_mean = class_embeddings_full[:prompt_num].mean(dim=0)
            class_embedding_mean /= class_embedding_mean.norm()
            
            text_feat.append(class_embedding_mean) ### 1024
            full_text_feat.append(class_embeddings_full[prompt_num:])
        ...
        max_len = max([i.shape[0] for i in full_text_feat])
        full_text_feat = [torch.cat([feat, torch.zeros(max_len - feat.shape[0], 512).cuda()], dim=0) for feat in full_text_feat]
        self.text_feat_full = torch.stack(full_text_feat, dim=0).cuda()
...
def dummy_get_fixed_clip(clip_arch, classnames, device, n_ctx, ctx_init, learned_cls=False, memory_size=10, text_prompt='tip'):

    model = Dummy_ClipFixed(device, classnames, None, arch=clip_arch, n_ctx=n_ctx, ctx_init=ctx_init, learned_cls=learned_cls, memory_size=memory_size, text_prompt=text_prompt)
